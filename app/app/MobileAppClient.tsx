...
export default function MobileAppClient({ 
  ...
  return (
    ...
    <div className="min-h-screen bg-[#EAE7E1] py-0 sm:py-8 flex justify-center selection:bg-[#0A2517] selection:text-white mobile-app-root font-sans">
      
      {/* 📱 KHUNG MÁY APP: TRẢI NGHIỆM 100% NATIVE MOBILE APP */}
      <div className="w-full max-w-[430px] min-h-screen sm:min-h-[890px] sm:max-h-[920px] bg-[#FBF9F5] text-[#0A2517] antialiased shadow-[0_25px_60px_rgba(0,0,0,0.18)] sm:rounded-[44px] sm:border-[6px] border-stone-800/80 relative overflow-y-auto overflow-x-hidden select-none pb-24 no-scrollbar flex flex-col font-sans">
        ...
        <div className="sticky top-0 z-40 bg-[#FBF9F5]/95 backdrop-blur-md pt-3.5 pb-2.5 px-4 border-b border-stone-200/60 transition-colors">
          ...
          {/* Tiêu đề linh hoạt theo từng tab */}
          <div className="flex items-center justify-between mb-2">
            <div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => { setActiveTab("home"); setSelectedOccasion("Tất cả"); }}
                  className="font-heading font-black text-2xl tracking-widest text-[#0A2517] uppercase text-left cursor-pointer"
                >
                  CLOOP
                </button>
                <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full bg-emerald-100/80 text-emerald-900 border border-emerald-200/60">
                  {activeTab === "shop" ? "Sàn đồ" : activeTab === "orders" ? "Đơn hàng" : activeTab === "closet" ? "Tủ cá nhân" : "Tuần hoàn"}
                </span>
              </div>
              <p className="text-[11px] text-stone-500 font-medium">
                {activeTab === "shop"
                  ? "Sàn thời trang tuần hoàn • Thuê & Sở hữu"
                  : activeTab === "orders"
                  ? "Lịch thuê & Theo dõi giao nhận trang phục"
                  : activeTab === "closet"
                  ? (displayName ? `Chào ${displayName} • Quản lý tủ đồ & doanh thu` : "Thời trang tuần hoàn CLOOP")
                  : (displayName ? `Chào ${displayName} • Tủ đồ chia sẻ` : "Chia sẻ tủ đồ • Mặc đẹp không lãng phí")}
              </p>
            </div>

            {/* Cụm nút Header: Nút 3-gạch (Drawer menu luôn hiển thị ở mọi tab) & Nút X (Về web) */}
            <div className="flex items-center bg-white border border-stone-200/90 rounded-full px-2.5 py-1 shadow-2xs">
              <button 
                type="button" 
                onClick={() => setIsDrawerMenuOpen(true)}
                className="text-stone-700 hover:text-[#0A2517] transition px-1 cursor-pointer flex items-center gap-1"
                title="Menu 3 gạch tính năng"
              >
                <Menu size={18} />
              </button>
              <div className="w-[1px] h-3.5 bg-stone-200 mx-1.5" />
              <Link 
                href="/" 
                className="text-stone-700 hover:text-[#0A2517] transition px-1"
                title="Về bản Web"
              >
                <X size={17} />
              </Link>
        ...
        {/* ========================================================
            ...
            ======================================================== */}
        <nav className="fixed bottom-0 left-0 right-0 max-w-[430px] mx-auto z-50 bg-white/95 backdrop-blur-md border-t border-stone-200/80 px-2 py-2 flex items-center justify-around shadow-lg pb-[calc(0.5rem+env(safe-area-inset-bottom,0px))] sm:rounded-b-[40px]">
