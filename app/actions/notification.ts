...
export async function getUserNotificationsAction(): Promise<{
  ...
  try {
    ...
    // ⚡ TRUY VẤN SONG SONG TẤT CẢ HOẠT ĐỘNG THỰC TẾ TRONG DATABASE
    const [
      escrowOrdersAsOwner,
    ...
    // ===== E. THÔNG BÁO CHÀO MỪNG NẾU LÀ NGƯỜI DÙNG MỚI =====
    if (notifList.length === 0) {
      const { formatted, relative } = formatDateTimeVN(new Date(user.created_at || Date.now()));
      notifList.push({
        id: "welcome-system-1",
        type: "SYSTEM",
