...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleFastLogin = async () => {
    ...
    try {
      ...
      alert(`Lỗi đăng nhập nhanh: ${err.message || err}`);
    } finally {
      setIsLoading(false);
    }
  };
              ...
              {(authMode === 'login' || authMode === 'register') && (
                <div className="space-y-2.5 pt-1">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-center gap-2.5 py-2.5 px-4 rounded-2xl border text-xs font-bold tracking-wide transition shadow-xs hover:bg-stone-50 active:scale-[0.99] disabled:opacity-50 ${
